"""Final gated pilot: second shortcut family (high-salience non-text semantic decoy).

This is the one final principled attempt at a stronger non-text second shortcut
family. It is isolated from the typographic text-overlay headline, the completed
scale-and-multi-model audit, the Waterbirds diagnostic, and the earlier
visual-decoy boundary result. It tests whether CIC region scoring can localize
and neutralize a *non-text, high-salience semantic* decoy icon among a fixed
...
transfer, exact localization, or universal shortcut repair. It is a single-model,
single-family controlled pilot. If gates fail it is retained only as boundary
evidence and second-family exploration stops permanently.
"""
...
from causal_reliability.data.clip_semantic_decoy_shortcuts import (
    SEMANTIC_DECOY_CLASSES,
    _corner_box,
    make_semantic_decoy_dataset,
    save_example_images,
)
...
PROMPT_TEMPLATE = "a simple colorful icon of a {label}"
MISLEADING_REGIME = "misleading_decoy"
CLEAN_REGIME = "no_decoy"
SHORTCUT_FAMILY = "non_text_semantic_decoy_icon"
...
def _metrics_table(certs: pd.DataFrame, status: ClipStatus, eligible: bool, reasons: list[str]) -> pd.DataFrame:
    rows = []
    for method in METHODS:
        ...
        row = {
            "method": method,
            "shortcut_family": SHORTCUT_FAMILY,
            "evidence_status": "pretrained CLIP semantic-decoy pilot evidence" if status.pretrained else "unavailable",
            "pilot_eligible": bool(eligible),
...
def _save_plot(out_dir: Path, v: dict[str, Any]) -> None:
    labels = [
        "misleading\noriginal",
        "matched\nrandom",
        "CIC top-1",
    ...
    vals = [
        v.get("misleading_original_accuracy"),
        v.get("matched_random_accuracy"),
        v.get("cic_top1_accuracy"),
        v.get("cic_top3_accuracy"),
        v.get("cic_clean_safe_accuracy"),
        v.get("oracle_repair_accuracy"),
    ]
    vals = [float(x) if x is not None else 0.0 for x in vals]
    colors = ["#b0b0b0", "#cc8855", "#2c7fb8", "#41a4c8", "#3fb37a", "#444444"]
    fig, ax = plt.subplots(figsize=(7.5, 4.2))
    bars = ax.bar(labels, vals, color=colors)
    ax.axhline(0.40, ls="--", lw=1, color="red", label="misleading-original gate (<=0.40)")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy (misleading regime)")
    ax.set_title("Semantic-decoy pilot: misleading-regime accuracy by method")
    for b, val in zip(bars, vals):
        ax.text(b.get_x() + b.get_width() / 2, val + 0.02, f"{val:.2f}", ha="center", va="bottom", fontsize=9)
    ax.legend(loc="upper left", fontsize=8)
    fig.tight_layout()
    fig.savefig(out_dir / "semantic_decoy_plot.png", dpi=130)
    plt.close(fig)


def _examples_md(out_dir: Path, examples: list[dict[str, Any]]) -> None:
    lines = [
        "# Semantic-Decoy Pilot: Example Stimuli",
        "",
        "Central icon = true label; corner icon = competing-class decoy (no text anywhere).",
        "Saved test-split example renders (subset). Paths are relative to this file.",
        "",
    ]
    by_regime: dict[str, list[dict[str, Any]]] = {}
    for ex in examples:
        by_regime.setdefault(ex["regime"], []).append(ex)
    for regime in ["no_decoy", "misleading_decoy", "aligned_decoy"]:
        items = by_regime.get(regime, [])[:4]
        if not items:
            continue
        lines.append(f"## Regime: `{regime}`")
        lines.append("")
        for ex in items:
            rel = Path(ex.get("image_path", "")).relative_to(out_dir) if ex.get("image_path") else None
            decoy = ex["decoy_class_name"] or "none"
            cap = f"true=`{ex['true_label']}` decoy=`{decoy}` corner=`{ex['corner']}`"
            if rel is not None:
                lines.append(f"- {cap}  \n  ![ex{ex['example_id']}]({rel})")
            else:
                lines.append(f"- {cap}")
        lines.append("")
    (out_dir / "semantic_decoy_examples.md").write_text("\n".join(lines), encoding="utf-8")
...
def _write_unavailable(out_dir: Path, cfg: dict[str, Any], status: ClipStatus) -> dict[str, str]:
        ...
        [
            {
                ...
                "eligibility_reasons": status.error_message or "pretrained CLIP did not load",
            }
        ]
    )
    metrics.to_csv(out_dir / "semantic_decoy_pilot_metrics.csv", index=False)
    pd.DataFrame([{"method": "unavailable"}]).to_csv(out_dir / "semantic_decoy_pilot_certificates.csv", index=False)
    (out_dir / "semantic_decoy_pilot_config_used.yaml").write_text(yaml.safe_dump(cfg, sort_keys=True), encoding="utf-8")
    gates = {"all_passed": False, "failed_gates": ["pretrained_loaded"], "values": {}, "gates": {"pretrained_loaded": False}}
    (out_dir / "semantic_decoy_pilot_gates.json").write_text(json.dumps(gates, indent=2, sort_keys=True), encoding="utf-8")
    (out_dir / "semantic_decoy_pilot_report.md").write_text(
        "\n".join(
            [
    ...
    (out_dir / "semantic_decoy_examples.md").write_text("# Semantic-Decoy Pilot: Example Stimuli\n\nPretrained CLIP unavailable; no stimuli scored.\n", encoding="utf-8")
    return {
        "metrics": str(out_dir / "semantic_decoy_pilot_metrics.csv"),
        "certificates": str(out_dir / "semantic_decoy_pilot_certificates.csv"),
        "gates": str(out_dir / "semantic_decoy_pilot_gates.json"),
        "report": str(out_dir / "semantic_decoy_pilot_report.md"),
    }


def _report_md(certs: pd.DataFrame, gate_result: dict[str, Any], status: ClipStatus, cfg: dict[str, Any], threshold_info: dict[str, Any]) -> str:
    ...
    recommendation = "SCALE to n=128 (pending explicit review/confirmation)" if passed else "STOP second-family exploration permanently"
    lines = [
        "# Semantic-Decoy Shortcut Pilot (second shortcut family, final attempt)",
        "",
        ...
        "",
        f"- Shortcut family: `{SHORTCUT_FAMILY}` (central coloured causal icon + competing-class corner icon, no written words)",
    ...
    if passed:
        ...
        lines += [
            ...
            "boundary evidence for the semantic-decoy shortcut family and is NOT integrated as a positive result.",
            "Second-family exploration stops permanently; no further tuning was performed to force a pass.",
        ]
    lines += [
        "",
...
def run(cfg: dict[str, Any]) -> dict[str, str]:
    ...
    out_dir = ensure_dir(Path(cfg.get("results_dir", "results")) / str(cfg.get("output_subdir", "semantic_decoy_pilot")))
    ...
    decoy_frac = float(data_cfg.get("decoy_frac", 0.46))
    object_frac = float(data_cfg.get("object_frac", 0.34))
    obj = (object_frac, object_frac)
    n_random = int(cfg.get("n_random_candidates", 3))
    max_clean_drop = float(cfg.get("max_clean_drop", 0.05))

    val_bundle = make_semantic_decoy_dataset(n_per_condition=val_n, size=size, regimes=regimes, split="validation", start_id=0, decoy_frac=decoy_frac, object_frac=obj)
    test_bundle = make_semantic_decoy_dataset(n_per_condition=test_n, size=size, regimes=regimes, split="test", start_id=len(val_bundle.examples), decoy_frac=decoy_frac, object_frac=obj)
    # Save a spread across regimes (not just the leading no_decoy block) so the
    # interesting misleading_decoy stimuli are rendered in the examples doc.
    sample_examples: list[dict[str, Any]] = []
    for regime in regimes:
        sample_examples.extend([ex for ex in test_bundle.examples if ex["regime"] == regime][:8])
    save_example_images(sample_examples, examples_dir, keys=["image", "clean_image"])
    ...
    if str(model_cfg.get("backend", "")).lower() == "fake" or requested_backend == "fake":
        status = ClipStatus(False, "fake", "fake_clip_semantic_decoy", pretrained=False, device=device, backend_attempted="fake", error_message="fake backend is not allowed for semantic-decoy pilot evidence")
    ...
    certs.to_csv(out_dir / "semantic_decoy_pilot_certificates.csv", index=False)
    metrics.to_csv(out_dir / "semantic_decoy_pilot_metrics.csv", index=False)
    (out_dir / "semantic_decoy_pilot_gates.json").write_text(json.dumps(gate_result, indent=2, sort_keys=True), encoding="utf-8")
    (out_dir / "semantic_decoy_pilot_config_used.yaml").write_text(yaml.safe_dump(cfg, sort_keys=True), encoding="utf-8")
    report = _report_md(certs, gate_result, status, cfg, threshold_info)
    (out_dir / "semantic_decoy_pilot_report.md").write_text(report, encoding="utf-8")
    _save_plot(out_dir, gate_result["values"])
    _examples_md(out_dir, test_bundle.examples)
    ...
    return {
        ...
        "gates": str(out_dir / "semantic_decoy_pilot_gates.json"),
        "report": str(out_dir / "semantic_decoy_pilot_report.md"),
        "plot": str(out_dir / "semantic_decoy_plot.png"),
        "examples": str(out_dir / "semantic_decoy_examples.md"),
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/semantic_decoy_pilot.yaml")
